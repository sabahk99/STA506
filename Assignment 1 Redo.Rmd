...
hist(times2, breaks = 3, freq = FALSE)
...
##  
my.den <- my.kerf(in.data = times2, h = 2, out.x = xx)
## density()
kde <- density(times2, bw = 2, kernel = "gaussian")
##
...
my.den <- my.kerf(in.data = times2, h = 2, out.x = xx)
## density()
kde <- density(times2, bw = 2, kernel = "epanechnikov")
##
kde.y <-approx(kde$x, kde$y, xout = xx)$y
## base R plot
plot(xx, my.den, type = "l", main = "Comparing Kernel Density Estimators",
...
##### We can see that choosing Gaussian kernel is favorable in terms of comparing my density estimator function with R's build in density estimator function. The Epanechnikov kernel results in a slight density difference at around time =20 between my estimator and R's estimator. 
...
{density(times2, bw = 1.5, adjust = 1,
        kernel = c("gaussian"),
        weights = NULL, window = kernel, width,
        give.Rkern = FALSE, subdensity = FALSE,
        warnWbw = var(weights) > 0,
...
xx = seq(0,40, length=200)
##  
my.den <- my.kerf(in.data = times2, h = 1.5, out.x = xx)
## density()
kde <- density(times2, bw = 1.5, kernel = "gaussian")
##
kde.y <-approx(kde$x, kde$y, xout = xx)$y
## base R plot
plot(xx, my.den, type = "l", main = "Comparing Kernel Density Estimators",
...
{density(times2, bw = 2.5, adjust = 1,
        kernel = c("gaussian"),
        weights = NULL, window = kernel, width,
        give.Rkern = FALSE, subdensity = FALSE,
        warnWbw = var(weights) > 0,
...
xx = seq(0,40, length=200)
##  
my.den <- my.kerf(in.data = times2, h = 2.5, out.x = xx)
## density()
kde <- density(times2, bw = 2.5, kernel = "gaussian")
##
kde.y <-approx(kde$x, kde$y, xout = xx)$y
## base R plot
plot(xx, my.den, type = "l", main = "Comparing Kernel Density Estimators",
...
my.den <- my.kerf(in.data = times2, h = 1.5, out.x = xx)
## density()
kde <- density(times2, bw = 1.5, kernel = "epanechnikov")
##
kde.y <-approx(kde$x, kde$y, xout = xx)$y
## base R plot
plot(xx, my.den, type = "l", main = "Comparing Kernel Density Estimators",
...
## density()
kde <- density(times2, bw = 2.5, kernel = "epanechnikov")
##
kde.y <-approx(kde$x, kde$y, xout = xx)$y
## base R plot
plot(xx, my.den, type = "l", main = "Comparing Kernel Density Estimators",
...
##### From the above graphs, we can see that the Gaussian kernel estimator is closest to my created density estimator function, regardless of the height used, but when an Epanechnikov kernel estimator is used, the curves look distinctly different, and it seems as h increases, the curves move further apart; this supports the idea that the bandwidth or height selection is more important than the type of kernel selected, because although the Gaussian estimator was better in all scenarios, the Epanechnikov estimator at h=1.5 was not too bad, but the one at h=2.5 is completely different. 